                ...
                <tbody>
                    ...
                    if (isset($resultado) && $resultado->num_rows > 0) {
                        while ($fila = mysqli_fetch_assoc($resultado)) { ?>
                           ...
                           <tr>
                        <td><?= !empty($fila['Tipo_Documento']) ? htmlspecialchars($fila['Tipo_Documento']) : 'No registra' ?></td>
                        <td><?= !empty($fila['Num_Documento']) ? htmlspecialchars($fila['Num_Documento']) : 'No registra' ?></td>
                        <td><?= !empty($fila['Nombres']) ? htmlspecialchars($fila['Nombres']) : 'No registra' ?></td>
                        <td><?= !empty($fila['Apellidos']) ? htmlspecialchars($fila['Apellidos']) : 'No registra' ?></td>
                        <td><?= !empty($fila['Teléfono']) ? htmlspecialchars($fila['Teléfono']) : 'No registra' ?></td>
                        <td><?= !empty($fila['Correo']) ? htmlspecialchars($fila['Correo']) : 'No registra' ?></td>
                       
                        <td>
                        <?php if (isset($fila['RolidRol']) && $fila['RolidRol'] !== null && $fila['RolidRol'] !== ''): ?>
                        <?= htmlspecialchars($fila['RolidRol']) ?>
                        <?php else: ?>
                        ...
                        <?= htmlspecialchars($fila['EstadoCodigoEstado']) ?>
                        <?php else: ?>
                        <span class="text-primary fw-bold"><i class="fas fa-info-circle"></i> Pendiente por administrador</span>
                        <?php endif; ?>
                        </td>     

                        <td><?= !empty($fila['GeneroidGenero']) ? htmlspecialchars($fila['GeneroidGenero']) : 'No suministra' ?></td>
                            ...
                            <td>
                                    <button class="btn btn-warning" data-bs-toggle="modal"
                                        data-bs-target="#updateModal<?= $fila['Num_Documento'] ?>">Editar</button>
                                </td>
                            </tr>
